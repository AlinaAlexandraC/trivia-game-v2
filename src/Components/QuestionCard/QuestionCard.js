            ...
            <div>
                ...
                <div className="answers-container d-flex flex-column justify-content-center align-content-center pt-3 pb-3">
                    {answersPool.length > 0 && questions[index] && answersPool.map((answer, answerIndex) => (
                        <div key={answerIndex} onClick={() => handleAnswerSelection()} className={`answer ${answer === questions[index].correct_answer ? 'correct-answer' : 'incorrect-answer'} d-flex flex-row justify-content-center align-content-center p-3`}>{HTMLDecode(answer)}</div>
                    ))}
                </div>
            </div>
        </div >
